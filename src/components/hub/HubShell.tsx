...
const navigation = [
  { href: "/hub", label: "My Apps", icon: Boxes },
  { href: "/hub/billing", label: "Wallet & Billing", icon: Wallet },
  { href: "/hub/api-keys", label: "Developer API Keys", icon: KeyRound },
  { href: "/hub/settings", label: "App Registry Settings", icon: Settings },
];
  ...
  return (
    <div className="flex h-screen w-full overflow-hidden bg-background text-zinc-100">
      ...
      <aside
        ...
        aria-label="VERZ Hub navigation"
      >
        <div>
          <div className="flex h-16 items-center justify-between border-b border-white/10 bg-black/40 px-5">
            <Link href="/hub" className="flex items-center" onClick={() => setMobileNavOpen(false)}>
              <VerzLogo size="sm" subtext="HUB" />
            </Link>
            <button
              type="button"
          ...
          <nav className="p-4" aria-label="Hub sections">
                ...
                return (
                  <li key={item.href}>
                    <Link
                      ...
                      onClick={() => setMobileNavOpen(false)}
                      className={`flex items-center gap-3 rounded-xl border px-3.5 py-2.5 font-semibold transition-colors ${
                        active
                          ? "border-cyan-400/35 bg-white/10 text-white"
                          : "border-transparent text-zinc-300 hover:bg-white/10 hover:text-white"
                      }`}
                    >
                      <Icon className={`h-4 w-4 ${active ? "text-cyan-400" : "text-zinc-400"}`} />
                      {item.label}
                    </Link>
                  </li>
                );
        ...
        <header className="flex h-16 shrink-0 items-center justify-between gap-3 border-b border-white/15 bg-black/60 px-3 backdrop-blur-md sm:px-6">
          <div className="flex min-w-0 items-center gap-3">
            ...
            <span className="truncate font-mono text-xs font-bold uppercase tracking-wider text-white">
              <span className="hidden lg:inline">VERZ Control Plane • </span>Operational
            </span>
          </div>
