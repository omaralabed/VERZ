    ...
    <div className="w-full bg-transparent text-white font-sans">
      <div className="max-w-7xl mx-auto space-y-8">
            ...
            {nodes.length === 0 ? (
              <div className="p-12 text-center rounded-2xl border border-white/10 bg-surface-100 space-y-4">
                ...
                <p className="text-sm text-zinc-400 max-w-sm mx-auto">
                  You do not have any cloud bonding relays deployed. Create your first dedicated instance on demand.
                </p>
                <button
                  onClick={() => setCurrentView("deploy")}
              ...
              <div className="space-y-3">
                <div className="grid gap-4 xl:grid-cols-2 2xl:hidden">
                  {nodes.map((node) => {
                    const activeDevCount = node.devices.filter((device) => device.status === "connected").length;
                    return (
                      <article key={node.id} className="rounded-2xl border border-white/15 border-l-4 border-l-emerald-400 bg-surface-100 p-4">
                        <div className="flex items-start justify-between gap-3">
                          <div className="min-w-0">
                            <div className="flex items-center gap-2">
                              <span className="h-2.5 w-2.5 shrink-0 rounded-full bg-emerald-400 shadow-[0_0_8px_#34d399]" />
                              <h2 className="truncate font-black text-white">{node.name}</h2>
                            </div>
                            <p className="mt-1 truncate font-mono text-xs text-zinc-500">{node.id}</p>
                          </div>
                          <span className="shrink-0 rounded-full border border-white/10 bg-white/5 px-2 py-1 font-mono text-[11px] text-zinc-300">
                            {node.category === "dedicated" ? "Dedicated" : "Shared"}
                          </span>
                        </div>

                        <dl className="mt-4 grid grid-cols-2 gap-3 border-y border-white/10 py-4 text-sm">
                          <div>
                            <dt className="font-mono text-[11px] uppercase text-zinc-500">Region</dt>
                            <dd className="mt-1 truncate font-bold text-white">{node.region}</dd>
                          </div>
                          <div>
                            <dt className="font-mono text-[11px] uppercase text-zinc-500">Public IP</dt>
                            <dd className="mt-1 font-mono font-bold text-cyan-300">{node.ipAddress}</dd>
                          </div>
                          <div>
                            <dt className="font-mono text-[11px] uppercase text-zinc-500">Devices</dt>
                            <dd className="mt-1 font-bold text-white">{activeDevCount} active · {node.devices.length}/{node.maxDevices}</dd>
                          </div>
                          <div>
                            <dt className="font-mono text-[11px] uppercase text-zinc-500">Rate</dt>
                            <dd className="mt-1 font-mono font-bold text-white">${node.hourlyPrice.toFixed(3)}/hr</dd>
                          </div>
                        </dl>
                        ...
                        <div className="mt-4 flex gap-2">
                          ...
                          <button
                            ...
                            className="rounded-xl border border-red-500/25 bg-red-500/10 p-2.5 text-red-400"
                          >
                ...
                <div className="hidden overflow-x-auto pb-4 2xl:block">
          ...
          <div className="space-y-6">
            ...
            <div className="flex items-center justify-between pb-4 border-b border-white/10">
              ...
              <div className="text-xs font-mono text-zinc-400">
                VERZ Cloud Backbone • Service status healthy
              </div>
            </div>
            ...
            <div>
              ...
              <p className="text-sm text-zinc-200 mt-1">
                Select relay capacity for multi-WAN bonding and device key generation.
              </p>
            </div>
            ...
            <div className="flex items-center gap-3 pt-2">
              <button
                onClick={() => {
                  ...
                  if (!selectedPlanId.startsWith("dedicated_")) setSelectedPlanId("dedicated_4gb");
                }}
              ...
              <button
                onClick={() => {
                  setActiveTab("shared");
                  if (!selectedPlanId.startsWith("shared_")) setSelectedPlanId("shared_1gb");
                }}
                className={`px-6 py-3 rounded-xl text-sm font-black font-mono transition-all ${
                  activeTab === "shared"
                    ? "bg-white text-black shadow-xl scale-[1.02]"
                  ...
                  return (
                    <button
                      ...
                      className={`rounded-2xl border p-4 text-left transition-colors ${
                        isSelected ? "border-cyan-400 bg-cyan-400/10" : "border-white/15 bg-surface-100"
                      }`}
                    >
                      <div className="flex items-start justify-between gap-3">
                        <div>
                          ...
                          <span className="mt-1 block font-mono text-xs text-zinc-400">
                            {plan.ram} · {plan.cpus} vCPU · {plan.transfer}
                          </span>
                        </div>
                        <span className={`mt-1 h-4 w-4 shrink-0 rounded-full border-2 ${isSelected ? "border-cyan-300 bg-cyan-300" : "border-white/40"}`} />
                      </div>
                      <div className="mt-4 flex items-end justify-between border-t border-white/10 pt-3">
                        <span className="font-mono text-xs font-bold text-emerald-300">Up to {plan.maxDevices} devices</span>
                        <span className="text-right">
                          <strong className="block text-lg text-white">${plan.monthlyPrice}/mo</strong>
                          <span className="font-mono text-xs text-cyan-300">${plan.hourlyPrice.toFixed(3)}/hr</span>
                        </span>
                      </div>
                    </button>
                  );
            ...
            <div className="hidden rounded-2xl border-2 border-white/20 bg-surface-100 overflow-hidden shadow-2xl 2xl:block">
            ...
            <div className="p-6 rounded-2xl bg-surface-100 border border-white/15 space-y-6">
              ...
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 pt-4 border-t border-white/10 font-mono">
                <div className="p-4 rounded-xl bg-black/40 border border-white/10">
                  ...
                  <span className="text-[11px] text-emerald-400">{activeNode.packetLossPct.toFixed(2)}% packet loss</span>
                </div>
                ...
                <div className="p-4 rounded-xl bg-black/40 border border-white/10">
                  ...
                  <span className="text-[11px] text-emerald-400">Session healthy</span>
                </div>
                            ...
                            <div className="flex flex-col md:flex-row items-center justify-between gap-4 pt-3 border-t border-white/10 font-mono text-xs">
                              ...
                              <div className="flex items-center gap-2 w-full md:w-auto justify-end">
                                ...
                                <code className="px-2 py-1 rounded bg-black/60 border border-white/15 text-zinc-300 text-[11px] font-mono">
                                  {revealedTokenIds.includes(device.id) ? device.pairingToken : "••••••••••••••••••••"}
                                </code>
                                <button
                                  type="button"
                                  onClick={() =>
                                  ...
                                  aria-label={`${revealedTokenIds.includes(device.id) ? "Hide" : "Reveal"} pairing token for ${device.name}`}
                                >
                                  {revealedTokenIds.includes(device.id) ? (
        ...
        {isAddDeviceOpen && activeNode && (
          <AccessibleDialog
            titleId="add-device-title"
            onClose={() => setIsAddDeviceOpen(false)}
            panelClassName="w-full max-w-md bg-surface-100 border-2 border-white/20 rounded-2xl p-6 sm:p-7 space-y-6 shadow-2xl"
          >
              <div className="flex items-center justify-between border-b border-white/10 pb-4">
                <h3 id="add-device-title" className="text-lg font-black text-white">Generate Device Connection Key</h3>
                <button
                  onClick={() => setIsAddDeviceOpen(false)}
                  className="text-zinc-400 hover:text-white"
                  aria-label="Close device key dialog"
                >
                  <X className="h-5 w-5" />
                </button>
              </div>
        ...
        {qrModalKey && (
          <AccessibleDialog
            titleId="qr-device-title"
            onClose={() => setQrModalKey(null)}
            panelClassName="w-full max-w-sm bg-surface-100 border border-white/20 rounded-2xl p-6 text-center space-y-6 shadow-2xl"
          >
              <div className="flex items-center justify-between border-b border-white/10 pb-3">
                <h3 id="qr-device-title" className="text-base font-bold text-white">{qrModalKey.name}</h3>
                <button
        ...
        {nodeToDelete && (
          <AccessibleDialog
            titleId="delete-relay-title"
            onClose={() => setNodeToDelete(null)}
            panelClassName="w-full max-w-lg bg-surface-100 border-2 border-red-500/40 rounded-2xl p-6 sm:p-8 space-y-6 shadow-[0_0_50px_rgba(239,68,68,0.25)] animate-in fade-in zoom-in-95 duration-150"
          >
              ...
              <div className="flex items-start gap-4">
                ...
                <div>
                  <h3 id="delete-relay-title" className="text-xl font-black text-white tracking-tight">
